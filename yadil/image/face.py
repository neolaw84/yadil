...
def _extract(img, f, size:Tuple=(256, 256), bbox_scale:float=1.0, correct_rotate:bool=False, return_all:bool=False):
    bbox = f["bbox"]
...
def extract(img, size:Tuple=(256, 256), bbox_scale:float=1.0, correct_rotate:bool=False, return_all:bool=False):
    ...
    rimgs = [_extract(img, f, size=size, bbox_scale=bbox_scale, correct_rotate=correct_rotate, return_all=return_all) for f in faces]
...
def _get_df_all(input_meta_glob):
    dfs = []
    for meta_fp in glob.glob(input_meta_glob):
...
def main(input_glob, output_dir, input_meta_glob=None, output_meta=None):
    if input_meta_glob:
        df_all = _get_df_all
    else:
        df_all = None
    ...
    def create_df(row_list):
        ...
        temp_df[["bbox_scaled_x1", "bbox_scaled_y1", "bbox_scaled_x2", "bbox_scaled_y2"]] = pd.DataFrame(temp_df.bbox_points.tolist(), index=temp_df.index)[[0, 1, 6, 7]]
    ...
    for f in glob.glob(input_glob):
        try:
            ...
            f_uuid = fname[:-4] if len(fname) > 4 else fname
            f_url = get_url(uuid_ = f_uuid) if df_all else ""
            img = cv2.imread(f)
            results = extract(img, bbox_scale=1.2, correct_rotate=True, return_all=True)
            ...
            if len(row_list) >= 1024:
                ...
                temp_df.to_csv(output_meta, header=False, index=False, mode="a+")
                row_list = []
